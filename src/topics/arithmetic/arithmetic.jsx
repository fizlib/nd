...
const generateMemberCheckProblem = () => {
    ...
    // const format = (num) => num.toString().replace('.', '{,}'); // Handled by Latex component now
    ...
    return {
        ...
        hints: [
            ...
            <span>Įstatykite žinomus skaičius: <Latex>{`${target} = ${a1} + (${d}) \\cdot (n-1)`}</Latex></span>,
            <span>
                Išspręskite lygtį <Latex>n</Latex> atžvilgiu:<br />
                <Latex>{`n - 1 = \\frac{${target} - ${a1}}{${d}} = ${(target - a1) / d}`}</Latex><br />
                <Latex>{`n = ${(target - a1) / d} + 1 = ${calcN}`}</Latex>.<br />
...
export default function ArithmeticTopic() {
                      ...
                      `}
                        ...
                        {problem.type === 'CHOICE' ? (
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    {problem.options.map((opt) => (
                                        <button
                                            ...
                                            disabled={feedback.type === 'success' || feedback.type === 'error' || revealedHints.length === problem.hints.length}
                                ...
                                {(revealedHints.length === problem.hints.length || feedback.type === 'error') && !isLevelComplete && (
                                    <button
                                        onClick={handleRepeat}
